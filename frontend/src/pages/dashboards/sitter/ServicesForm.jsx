...
function Toggle({ active, onChange }) {
  return (
    <div role="switch" aria-checked={active}
...
function ServiceCard({ icon, title, serviceKey, services, setServices }) {
  ...
  const handleToggle = (v) => setServices(p => ({ ...p, [serviceKey]: { ...s, active: v }}));
  const handlePriceChange = (e) => setServices(p => ({ ...p, [serviceKey]: { ...s, price: parseFloat(e.target.value)||0 }}));
  return (
    <div className={`p-6 bg-white rounded-xl shadow-md transition-opacity ${!s.active && 'opacity-50'}`}>
      ...
      <div className="mt-6">
        <label className="block text-sm font-medium text-gray-600 mb-2">Preço (R$)</label>
        <div className="relative">
          <span className="absolute inset-y-0 left-0 pl-3 flex items-center text-gray-500">R$</span>
          <input type="number" value={s.price} onChange={handlePriceChange} disabled={!s.active}
            className="w-full pl-10 pr-4 py-2 border border-gray-300 rounded-lg disabled:bg-gray-100" min="0" step="0.01"/>
        </div>
      </div>
    </div>
  );
...
export default function ServicesForm() {
  ...
  useEffect(() => {
    getMyServices().then(setServices).catch(() => setServices({
      walking:{active:false,price:0}, sitting:{active:false,price:0}, hosting:{active:false,price:0}
    }));
  }, []);
    ...
    <div className="bg-slate-50 min-h-screen p-4 sm:p-8">
      <div className="max-w-4xl mx-auto">
        ...
        <form className="space-y-6" onSubmit={onSubmit}>
          ...
          <ServiceCard icon={<PawPrint size={24}/>} title="Babá"  serviceKey="sitting" services={services} setServices={setServices}/>
          ...
          <div className="flex justify-end pt-4">
            <button type="submit" disabled={saving}
              ...
              {saving ? 'Salvando...' : 'Guardar Alterações'}
            </button>
          </div>
        </form>

        {feedback && <div className="mt-4 text-center text-green-700">{feedback}</div>}
      </div>
    </div>
  );
}
